...
// TODO: Adapt this code to ARM asm syntax
...
// Max number of floating point registers
#define FP_MAX 8
...
// Registers used for 8-bit arguments
static char *argreg8[] = {"%dil", "%sil", "%dl", "%cl", "%r8b", "%r9b"};

// Registers used for 16-bit arguments
static char *argreg16[] = {"%di", "%si", "%dx", "%cx", "%r8w", "%r9w"};

// Registers used for 32-bit arguments
static char *argreg32[] = {"%edi", "%esi", "%edx", "%ecx", "%r8d", "%r9d"};

// Registers used for 64-bit arguments
static char *argreg64[] = {"%rdi", "%rsi", "%rdx", "%rcx", "%r8", "%r9"};

// Current function object
static Obj *current_fn;

// Generate an expression given a node
static void gen_expr(Node *node);

// Generate a statement given a node
...
// Return the corresponding dx register for the given size
...
// Return the corresponding ax register for the given size
...
// Compute the abs address of a given node.
// If the node does not reside in mem, error.
static void gen_addr(Node *node) {
    switch (node->kind) {
        case ND_VAR:
            // Variable length array, always local
            ...
            if (node->ty->kind == TY_FUNC) {
                if (node->var->is_definition) println("  lea %s(%%rip), %%rax", node->var->name);
                else println("  mov %s@GOTPCREL(%%rip), %%rax", node->var->name);
                return;
            }
            // Global variable
        ...
        case ND_MEMBER:
            gen_addr(node->lhs);
            println("  add $%d, %%rax", node->member->offs);
            return;
        case ND_FUNCALL:
            if (node->ret_buffer) {
                gen_expr(node);
    ...
    error_tok(node->tok, EXPECTED_LVALUE);
}

// Load a value from %rax ptr
static void load(Type *ty) {
    switch (ty->kind) {
        case TY_ARRAY: case TY_STRUCT: case TY_UNION: case TY_FUNC: case TY_VLA:
...
static void store(Type *ty) {
    ...
    if (ty->size == 1) println("  movss %%xmm0, (%%rdi)");
    else if (ty->size == 2) println("  mov %%ax, (%%rdi)");
    else if (ty->size == 4) println("  mov %%rax, (%%rdi)");
}

// Compare a value to 0
static void cmp_zero(Type *ty) {
    ...
    if (is_int(ty) && ty->size <= 4) println("   cmp $0, %%eax");
...
// The table of ASM operation for type casts
...
// Cast table, defining the conversion from one type to another
...
// Cast from `from` type to `to` type
...
static bool has_flonum(Type *ty, int lo, int hi, int offs) {
    if (ty->kind == TY_STRUCT || ty->kind == TY_UNION) {
        for (Member *mem = ty->members; mem; mem = mem->next) {
            if (!has_flonum(mem->ty, lo, hi, offs + mem->offs)) return false;
        }
        return true;
    }
    return offs < lo || hi <= offs ||ty->kind == TY_FLOAT || ty->kind == TY_DOUBLE;
}

// Returns true if `ty` has only floating-point members in its byte range [0, 8]
static bool has_flonum1(Type *ty) {
    return has_flonum(ty, 0, 8, 0);
}

// Returns true if `ty` has only floating-point members in its byte range [8, 16]
static bool has_flonum2(Type *ty) {
    return has_flonum(ty, 8, 16, 0);
}

// Push struct of type `ty` on the stack
...
// Push function call arguments to the stack
...
static int push_args(Node *node) {
    ...
    // If a return type is a large struct/union, the caller passes
    // a pointer to the buf as first arg.
...
// Copy the return value of a function to the return buffer
...
static void gen_expr(Node *node) {
    ...
    switch (node->kind) {
        ...
        case ND_MEMBER: {
            ...
            if (mem->is_bitfield) {
                println("  sh1 $%d, %%rax", 64 - mem->bit_width - mem->bit_offs);
                if (mem->ty->is_unsigned) println("  shr $%d, %%rax", 64 - mem->bit_width);
                else println("  sar $%d, %%rax", 64 - mem->bit_width);
            }
            return;
        }
        case ND_DEREF:
        ...
        case ND_ASSIGN:
            ...
            if (node->lhs->kind == ND_MEMBER && node->lhs->member->is_bitfield) {
                println("  mov %%rax, %%r8");
                // If lhs is bitfield, need to read cur val
                // from mem and merge with new val
                Member *mem = node->lhs->member;
                println("  mov %%rax, %%rdi");
                println("  and $%ld, %%rdi", (1L << mem->bit_width) - 1);
                println("  shl $%d, %%rdi", mem->bit_offs);
                println("  mov (%%rsp), %%rax");
                load(mem->ty);
                long mask = ((1L << mem->bit_width) - 1) << mem->bit_offs;
        ...
        case ND_FUNCALL: {
            ...
            gen_expr(node->lhs);
            int gp = 0, fp = 0;
            // If return type is large struct/union, caller passes ptr
            // to a buf as first arg.
            ...
            depth -= stack_args;
            // It looks like the ms 48-56 bits in RAX may
            // contain garbage if func return type is short or bool/char,
            ...
            // If return type is small struct, a val returned using ->2 registers.
            if (node->ret_buffer && node->ty->size <= 16) {
    ...
    switch (node->lhs->ty->kind) {
        case TY_FLOAT: case TY_DOUBLE: {
            ...
            gen_expr(node->lhs);
            popf(1);
            char *sz = (node->ty->kind == TY_FLOAT) ? "ss" : "sd";
            switch (node->kind) {
                case ND_ADD:
    ...
    switch (node->kind) {
        ...
        case ND_DIV: case ND_MOD:
            if (node->ty->is_unsigned) {
                ...
                if (node->lhs->ty->size == 8) println("  cqo");
                else println("  cdq");
                println("  idiv %s", di);
            }
            if (node->kind == ND_MOD) println("  mod %%rdx, %%rax");
            return;
        case ND_BITAND:
        ...
        case ND_EQ: case ND_NE: case ND_LT: case ND_LE:
            ...
            if (node->kind == ND_EQ) {
                ...
                if (node->lhs->ty->is_unsigned) println("  setb %%al");
                else println("  setl %%al");
            } else if (node->kind == ND_LE) {
                if (node->lhs->ty->is_unsigned) println("  setbe %%al");
        ...
        case ND_SHR:
            println("  mov %%rdi, %%rcx");
            if (node->lhs->ty->is_unsigned) println("  shr %%cl, %s", ax);
            else println("  sar %%cl, %s", ax);
            return;
        default: break;
    }
    error_tok(node->tok, INV_EXPR);
...
static void assign_lvar_offsets(Obj *prog) {
    for (Obj *fn = prog; fn; fn = fn->next) {
        ...
        // If a func has many params, some are inevitably
        // passed by stack rather than by register.
        // The first passed by stack is at RBP+16.
        int top = 16;
        int bottom = 0;
        int gp = 0, fp = 0;
        ...
        // Assign offset to pbr params and local vars
        for (Obj *var = fn->locals; var; var = var->next) {
            if (var->offset) continue;
            // AMD64 System V ABI >16B align rule
            int align = (var->ty->kind == TY_ARRAY && var->ty->size >= 16) ? MAX(16, var->align) : var->align;
            bottom += var->ty->size;
...
// Emits data given by `prog`
static void emit_data(Obj *prog) {
    for (Obj *var = prog; var; var = var->next) {
        if (var->is_function || !var->is_definition) continue;
        if (var->is_static) println("  .local %s", var->name);
        else println("  .global %s", var->name);
        int align = (var->ty->kind == TY_ARRAY && var->ty->size >= 16) ? MAX(16, var->align) : var->align;
        // Common symbol
        ...
        // .bss or .tbss
        if (var->is_tls) println("  .section .tbss,\"awT\",@nobits");
        else println("  .bss");
        println("  .align %d", align);
        println("%s:", var->name);
...
// Stores a float or double in the right register
...
// Stores a non-floating point value in the right register
...
// Emits text given by `prog`
static void emit_text(Obj *prog) {
    for (Obj *fn = prog; fn; fn = fn->next) {
        if (!fn->is_function || !fn->is_definition) continue;
        // No code is emitted for "static inline" func if no one refs them.
        ...
        gen_stmt(fn->body);
        assert(depth == 0);
        // The C spec defines special rule for main func.
        // Reaching end of main func is equivalent to return 0,
        // even though behavior undefined for other funcs.
...
// Generate ASM code given a program and a target file
