...
// TODO: Substitute c types with ace types

#include "acec.h"
#include "../../definitions/errmsg.h"

/// Types definition, sizes and alignments and if they are signed or unsigned
...
/// Create a pointer version of the given type
...
/// Create a function type of the given return type
...
/// Create an array type of the given base type and length
...
/// Create a variable length array type of the given base type and length
Type *is_vla_of(Type *base, Node *len) {
...
/// Get the common type of two types if they are compatible
    ...
    switch (node->kind) {
        ...
        case ND_ASSIGN:
            if (node->lhs->ty->kind == TY_ARRAY)
                error_tok(node->lhs->tok, EXPECTED_LVALUE);
        ...
        case ND_FUNCALL:
            ...
            return;
        case ND_NOT: case ND_LOGOR: case ND_LOGAND:
            node->ty = ty_int;
            return;
        case ND_BITNOT: case ND_SHL: case ND_SHR:
            node->ty = node->lhs->ty;
            return;
        case ND_VAR: case ND_VLA_PTR:
            node->ty = node->var->ty;
            return;
        case ND_COND:
        ...
        case ND_DEREF:
            if (!node->lhs->ty->base)
                error_tok(node->tok, CANNOT_DEFERENCE_PTR);
            if (node->lhs->ty->base->kind == TY_VOID)
                error_tok(node->tok, CANNOT_DEFERENCE_VOID);
        ...
        case ND_STMT_EXPR:
            ...
            error_tok(node->tok, CANNOT_RET_VOID_STMT);
        case ND_LABEL_VAL:
        ...
        case ND_CAS:
            ...
            if (node->cas_addr->ty->kind != TY_PTR)
                error_tok(node->cas_addr->tok, EXPECTED_PTR);
            if (node->cas_old->ty->kind != TY_PTR)
                error_tok(node->cas_old->tok, EXPECTED_PTR);
            return;
        case ND_EXCH:
